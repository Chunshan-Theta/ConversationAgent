...
class CleanStage(Stage):
    ...
    def __init__(self):
        ...
        self.sys_reply_q1 = f"""
            ...
            在
             {StageMemoryFragOperation. get_default_frag_label(OrderEndStage.OUT_BOARD)} 下車
            如果想重新體驗，請輸入『我要重置』
        """
...
class OrderStartStage(Stage):
    ...
    def __init__(self):
        ...
        self.sys_reply_complete = f"好的，您將從 {StageMemoryFragOperation. get_default_frag_label(self.ON_BOARD)} 出發"

    def is_fit_needs_n_gen_entity(self, kwargs) -> Tuple[bool, dict]:
        user_text = kwargs.get(__USER_TEXT__, "")
        if fit_re(".*車站.*", user_text):
            kwargs = StageMemoryFragOperation. set_default_frag(kwargs, self.ON_BOARD, user_text)
            return True, kwargs
        else:
            return False, kwargs
...
class OrderEndStage(Stage):
    ...
    def __init__(self):
        ...
        self.sys_reply_q2 = "請說站名，例如『板橋火車站』"
        self.sys_reply_complete = f"好的，您將從 {StageMemoryFragOperation. get_default_frag_label(self.OUT_BOARD)} 下車"

    def is_fit_needs_n_gen_entity(self, kwargs) -> Tuple[bool, dict]:
        user_text = kwargs.get(__USER_TEXT__, "")
        if fit_re(".*車站.*", user_text):
            kwargs = StageMemoryFragOperation. set_default_frag(kwargs, self.OUT_BOARD, user_text)
            return True, kwargs
        else:
            return False, kwargs
...
class OrderConfirmStage(Stage):
    ...
    def __init__(self):
        ...
        self.sys_reply_complete = f"好的，您完成訂票了\n {StageMemoryFragOperation. get_default_frag_label(OrderStartStage.ON_BOARD)} 上車\n {StageMemoryFragOperation. get_default_frag_label(OrderEndStage.OUT_BOARD)} 下車"
